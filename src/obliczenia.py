...
def count_daily_avg(df, cities=None):
    """
    Oblicza średnie dobowe stężenia PM2.5 dla każdej miejscowości i roku.
    Args:
        df (pd.DataFrame): DataFrame z kolumnami 'stacja', 'miejscowość', 'rok', 'czas', 'wartość'.

    Returns:
        pd.DataFrame: DataFrame ze średnimi dobowymi stężeniami.
                      Kolumny: ['stacja', 'rok', 'miejscowość', 'data', 'pm25_dobowe'].
    """
    # Filtrowanie danych
    df = filter_data(df)

    # Filtr po miastach tylko jeśli podano listę
    if cities and len(cities) > 0:
        df = df[df['miejscowość'].isin(cities)]

    df['data'] = df['czas'].dt.date

    # Tabela dobowych stężeń dla wszystkich stacji wynikająca z jednostkowych pomiarów.
    daily_avg = (df.groupby(['rok', 'miejscowość', df['czas'].dt.date])
                ['wartość'].mean().reset_index()
                .rename(columns={'czas': 'data', 'wartość': 'pm25_dobowe'}))

    return daily_avg


def count_daily_avg2(df):
    """
    Oblicza średnie dobowe stężenia PM2.5 dla każdej stacji, miejscowości i roku.
    Args:
        df (pd.DataFrame): DataFrame z kolumnami 'stacja', 'miejscowość', 'rok', 'czas', 'wartość'.
    ...
    # Tabela dobowych stężeń dla wszystkich stacji wynikająca z jednostkowych pomiarów.
    df['data'] = df['czas'].dt.date
    daily_avg = (df.groupby(['stacja', 'rok', 'miejscowość', 'data'])
                ['wartość'].mean().reset_index()
                .rename(columns={'czas': 'data', 'wartość': 'pm25_dobowe'}))
...
#ZADANIE 4
def count_exceedances(df, target_year, cities=None):
    """
    ...
    daily_avg = count_daily_avg2(df)

    # Jeśli miejscowości były podane: filtr
    if cities:
        daily_avg = daily_avg[daily_avg['miejscowość'].isin(cities)]
    ...
    # Filtr po latach
    exceedances = exceedances[exceedances['rok'] == target_year]

    # Wyznaczenie 3 stacji o największej liczbie wykroczeń w roku 2024 i 3 o najmniejszej
    # max_3 = exceedances.nlargest(3, 'ilość przekroczeń')
    # min_3 = exceedances.nsmallest(3, 'ilość przekroczeń')
    #
...
def save_analysis_results(df, output_dir, year):
    """
    Funkcja sterująca, która wywołuje obliczenia i zapisuje wyniki do folderu.
    """
    # Stworzenie folderu na wyniki PM2.5
    os.makedirs(output_dir, exist_ok=True)

    # Generowanie i zapis średnich dobowych stężeń PM2.5 dla miast w danym roku
    df_daily = count_daily_avg(df)
    df_daily.to_csv(os.path.join(output_dir, "daily_means.csv"), index=False)

    # Generowanie i zapis statystyk przekroczeń normy stężenia PM2.5 dla wszystkich stacji w danym roku
    df_exc = count_exceedances(df, year)
    df_exc.to_csv(os.path.join(output_dir, "exceedance_days.csv"), index=False)

    print(f"Utworzono pliki exceedances_days.csv i daily_means.csv PM2.5 dla roku {year} w folderze {output_dir}")



if __name__ == "__main__":

    # Odbiornik dla danych od Snakemake
    ...
    # Wczytanie danych (cały złączony df)
    df_all = pd.read_pickle(args.input)

    # Dodanie kolumny miesiąc
    df_all = add_month_column(df_all)

    # Filtr po latach
    df_selected = df_all[df_all['rok'] == args.year]

    # Zabezpieczenie przed pustymi znakami
    if args.cities: # and any(c.strip() for c in args.cities):
        df_selected = df_selected[df_selected['miejscowość'].isin(args.cities)]
        print(f"Filtrowanie zakończone. Wybrane miasta: {args.cities}")
    else:
        print("Brak miast w configu - analiza wszystkich dostępnych miejscowości.")

    # Zapisanie plików exceedances_days, daily_avg
    save_analysis_results(df_selected, args.output_dir, args.year)